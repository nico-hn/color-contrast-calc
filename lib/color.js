...
class LightnessFinder {
  ...
  static boundaryColor(color, max, min, level) {
    if (min === 0 && ! this.hasSufficientContrast(color.BLACK.relativeLuminance,
						  color.rgb, level)) {
      return color.BLACK;
    }

    if (max === 100 && ! this.hasSufficientContrast(color.WHITE.relativeLuminance,
						    color.rgb, level)) {
      return color.WHITE;
    }
   ...
   * @private
   */
  static hasSufficientContrast(fixedLuminance, rgb, level) {
    const targetRatio = Checker.levelToRatio(level);
    const luminance = Checker.relativeLuminance(rgb);
