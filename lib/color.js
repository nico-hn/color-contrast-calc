...
/** @private */
const COLOR_KEY_WORDS = require("./color-keywords.json");
...
class Color {
   ...
   * @returns {Color}
   */
  static getByRgb(rgb, name = null) {
    const hexCode = Utils.rgbToHexCode(rgb);
   ...
   * @returns {Color}
   */
  static getByHexCode(code, name = null) {
   ...
   */
  static newHslColor(hsl, name = null) {
  ...
  constructor(rgb, name = null) {
     ...
     */
    this.name = name === null ? this.hexCode : name;
   ...
   */
  get commonName() {
    const namedColor = Color.List.HEX_TO_COLOR.get(this.hexCode);
    return namedColor && namedColor.name || this.hexCode;
  }
...
class List {
   ...
   * @private
   */
  static loadColorKeywords(colorKeyWords) {
    /**
