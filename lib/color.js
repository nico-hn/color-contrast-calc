...
class LightnessFinder {
  ...
  static find(fixedColor, otherColor, level = "AA") {
    ...
    const [max, min] = SearchCriteria.shouldScanDarkerSide(fixedColor, otherColor) ? [initL, 0] : [100, initL];
    const boundaryColor = this.boundaryColor(fixedColor, max, min, level);
    ...
    for (let d of Color.calc.binarySearchWidth(max - min, 0.01)) {
      ...
      const contrastRatio = fixedColor.contrastRatioAgainst(newColor);
...
class Color {
  ...
  findLightnessThreshold(otherColor, level = "AA") {
    return LightnessFinder.find(this, otherColor, level);
  }
