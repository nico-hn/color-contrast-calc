...
class BrightnessFinder extends ThresholdFinder {
  ...
  static find(fixedColor, otherColor, level = "AA") {
    ...
    const upperRgb = Utils.BrightnessCalc.calcRgb(otherColor.rgb, w * 2);

    if (this.exceedUpperLimit(fixedColor, criteria, otherColor, upperRgb)) {
      return new Color(upperRgb);
    }

    const [r, lastSufficentRatio] = this.calcBrightnessRatio(otherColor.rgb, criteria, w);

    const nearestRgb = Utils.BrightnessCalc.calcRgb(otherColor.rgb, criteria.round(r));

    if (lastSufficentRatio && ! criteria.hasSufficientContrast(nearestRgb)) {
      return new Color(Utils.BrightnessCalc.calcRgb(otherColor.rgb,
						    criteria.round(lastSufficentRatio)));
    }
   ...
   * @private
   */
  static exceedUpperLimit(fixedColor, criteria, otherColor, upperRgb) {
    return otherColor.isBrighterThan(fixedColor) && ! criteria.hasSufficientContrast(upperRgb);
  }
