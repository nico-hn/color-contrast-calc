...
class LightnessFinder extends ThresholdFinder {
  ...
  static find(fixedRgb, otherRgb, level = "AA") {
    ...
    const nearestRgb = this.rgbWithRatio(otherHsl, l);

    if (lastSufficientLightness && ! criteria.hasSufficientContrast(nearestRgb)) {
      return this.rgbWithRatio(otherHsl, lastSufficientLightness);
    }
   ...
   * @private
   */
  static findRatio(otherHsl, criteria, initRatio, initWidth) {
    let l = initRatio;
    let lastSufficientLightness = null;

    for (let d of this.binarySearchWidth(initWidth, 0.01)) {
      const newRgb = this.rgbWithRatio(otherHsl, l);
    ...
    return [l, lastSufficientLightness];
  }
}
...
class BrightnessFinder extends ThresholdFinder {
  ...
  static find(fixedRgb, otherRgb, level = "AA") {
    ...
    const [r, lastSufficentRatio] = this.calcBrightnessRatio(otherRgb, criteria, w);
