...
class SearchCriteria {
  static shouldScanDarkerSide(fixedColor, otherColor) {
    return fixedColor.isBrighterThan(otherColor) ||
      fixedColor.hasSameLuminance(otherColor) && fixedColor.isLightColor();
  }

  static thresholdCriteria(fixedColor, otherColor, level) {
    const targetRatio = Checker.levelToRatio(level);

    if (this.shouldScanDarkerSide(fixedColor, otherColor)) {
      return new ToDarkerSide(targetRatio, fixedColor.rgb);
    } else {
      return new ToBrighterSide(targetRatio, fixedColor.rgb);
    }
  }
...
/** @private */
class LightnessFinder {
  /**
   * @private
   */
  static boundaryColor(color, max, min, level) {
    if (min === 0 && ! color.hasSufficientContrast(color.BLACK, level)) {
      return color.BLACK;
    }

    if (max === 100 && ! color.hasSufficientContrast(color.WHITE, level)) {
      return color.WHITE;
    }
...
class Color {
  ...
  findBrightnessThreshold(otherColor, level = "AA") {
    const criteria = SearchCriteria.thresholdCriteria(this, otherColor, level);
    const w = BrightnessFinder.calcUpperRatioLimit(otherColor) / 2;
    ...
    const [r, lastSufficentRatio] = this.calcBrightnessRatio(otherColor, criteria, w);
  ...
  findLightnessThreshold(otherColor, level = "AA") {
    ...
    const [max, min] = SearchCriteria.shouldScanDarkerSide(this, otherColor) ? [initL, 0] : [100, initL];
    const boundaryColor = LightnessFinder.boundaryColor(this, max, min, level);
    ...
    for (let d of Color.calc.binarySearchWidth(max - min, 0.01)) {
      const newColor = Utils.hslToRgb([h, s, l]);
      const contrastRatio = this.contrastRatioAgainst(newColor);

      if (criteria.hasSufficientContrast(newColor)) {
    ...
    if (lastSufficientLightness && ! criteria.hasSufficientContrast(nearlestColor.rgb)) {
   ...
   * @private
   */
  calcBrightnessRatio(otherColor, criteria, w) {
