...
class BrightnessFinder extends ThresholdFinder {
  ...
  static find(fixedRgb, otherRgb, level = "AA") {
    ...
    const upperRgb = Utils.BrightnessCalc.calcRgb(otherRgb, w * 2);
    ...
    const nearestRgb = Utils.BrightnessCalc.calcRgb(otherRgb, criteria.round(r));

    if (lastSufficentRatio && ! criteria.hasSufficientContrast(nearestRgb)) {
      return new Color(Utils.BrightnessCalc.calcRgb(otherRgb,
  ...
  static calcUpperRatioLimit(rgb) {
    ...
    if (Utils.isSameRgbColor(black, rgb)) {
      return 100;
    }
...
class Color {
  ...
  findBrightnessThreshold(otherColor, level = "AA") {
    return BrightnessFinder.find(this.rgb, otherColor.rgb, level);
  }
