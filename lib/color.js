...
class LightnessFinder extends ThresholdFinder {
  ...
  static find(fixedRgb, otherRgb, level = "AA") {
    ...
    const [l, lastSufficientLightness] = this.findRatio(otherHsl, criteria,
                                                        (max + min) / 2, max - min);

    return this.rgbWithBetterRatio(otherHsl, criteria, l, lastSufficientLightness);
  }
   ...
   * @private
   */
  static rgbWithBetterRatio(otherColor, criteria, l, lastSufficientRatio) {
    const nearestRgb = this.rgbWithRatio(otherColor, l);

    if (lastSufficientRatio && ! criteria.hasSufficientContrast(nearestRgb)) {
      return this.rgbWithRatio(otherColor, lastSufficientRatio);
    }
...
class BrightnessFinder extends ThresholdFinder {
  ...
  static find(fixedRgb, otherRgb, level = "AA") {
    ...
    const [r, lastSufficentRatio] = this.findRatio(otherRgb, criteria, w, w);

    const nearestRgb = this.rgbWithRatio(otherRgb, criteria.round(r));

    if (lastSufficentRatio && ! criteria.hasSufficientContrast(nearestRgb)) {
      return this.rgbWithRatio(otherRgb, criteria.round(lastSufficentRatio));
    }
