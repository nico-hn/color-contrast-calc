...
class LightnessFinder {
  ...
  static find(fixedColor, otherColor, level = "AA") {
    ...
    const boundaryRgb = this.boundaryColor(fixedColor, max, min, criteria);

    if (boundaryRgb) { return new Color(boundaryRgb); }
   ...
   * @private
   */
  static boundaryColor(color, max, min, criteria) {
