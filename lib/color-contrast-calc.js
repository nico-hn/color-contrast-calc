...
(function() {
  class Sorter {
    ...
    static compareColorFunction(colorOrder = "rgb") {
      ...
      return function(color1, color2) {
        return Sorter.compareRgbVal(color1.rgb, color2.rgb, order.pos, order.funcs);
      };
    }
    ...
    static compareRgbFunction(colorOrder = "rgb") {
      ...
      return function(rgb1, rgb2) {
        return Sorter.compareRgbVal(rgb1, rgb2, order.pos, order.funcs);
      };
    }
    ...
    static compareHexFunction(colorOrder = "rgb") {
      ...
      return function(hex1, hex2) {
        return Sorter.compareHexVal(hex1, hex2, order, rgbCache);
      };
    }

    static compareRgbVal(rgb1, rgb2, rgbPos = [0, 1, 2],
                         compFuncs = this.defaultCompFuncs) {
      return this.compareColorComponents(rgb1, rgb2, rgbPos, compFuncs);
    }
    ...
    static compareHexVal(hex1, hex2, order, rgbCache) {
      const rgb1 = rgbCache.get(hex1) || order.toComponents(hex1);
      const rgb2 = rgbCache.get(hex2) || order.toComponents(hex2);

      return this.compareRgbVal(rgb1, rgb2, order.pos, order.funcs);
    }
    ...
    static parseColorOrder(colorOrder) {
      return {
        pos: this.primaryColorPos(colorOrder),
