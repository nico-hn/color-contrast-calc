...
class ColorContrastCalc {
  ...
  findBrightnessThreshold(otherColor, level = "AA") {
    ...
    const ratioLimit = this.calcUpperRatioLimit();
    const criteria = this.brightnessThresholdCriteria(targetRatio, ratioLimit, otherColor);
    const w = ratioLimit / 2;
    let r = w;
  ...
  brightnessThresholdCriteria(targetRatio, ratioLimit, otherColor) {
    const criteria = {};

    if (this.relativeLuminance > otherColor.relativeLuminance) {
      criteria.round = (r) => Math.floor(r);
      criteria.incrementCondition = (contrastRatio) => contrastRatio > targetRatio;
    } else if (this.relativeLuminance < otherColor.relativeLuminance) {
      criteria.round = (r) => Math.ceil(r);
      criteria.incrementCondition = (contrastRatio) => targetRatio > contrastRatio;
    } else {
      if (this.WHITE.contrastRatioAgainst(this) <= this.BLACK.contrastRatioAgainst(this)) {
        criteria.round = (r) => Math.floor(r);
        criteria.incrementCondition = (contrastRatio) => contrastRatio > targetRatio;
      } else {
        criteria.round = (r) => Math.ceil(r);
