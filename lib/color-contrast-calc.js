...
class ColorContrastCalc {
  ...
  findBrightnessThreshold(otherColor, level = "AA") {
    ...
    const lowLimit = (ratioLimit === 100) ? 0 : 100;
    const w = (ratioLimit - lowLimit) / 2;
    let r = lowLimit + w;

    for (let i = 1, d = w / Math.pow(2, i); d >= 0.1; i++, d = w / Math.pow(2, i)) {
      ...
      if (ratioLimit === 100 ) {
        r += contrastRatio > targetRatio ? d : -d;
      } else {
        r += targetRatio > contrastRatio ? d : -d;
      }
    }

    r = ratioLimit === 100 ? Math.floor(r) : Math.ceil(r);
    return otherColor.newBrightnessColor(r);
  }
