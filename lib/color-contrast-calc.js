...
(function() {
  class Sorter {
    ...
    static guessKeyType(mode, color, keyMapper) {
      ...
      } else if (mode === this.KEY_TYPE.RGB || Array.isArray(color)) {
        return this.KEY_TYPE.RGB;
      } else if (mode === this.KEY_TYPE.HSL) {
        return this.KEY_TYPE.HSL;
      } else {
    ...
    static compareHexVal(hex1, hex2, order, rgbCache) {
      ...
      return this.compareColorComponents(rgb1, rgb2, order.pos, order.funcs);
    }
