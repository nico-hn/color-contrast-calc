...
class Color {
  ...
  static newHslColor(hsl) {
    return ColorContrastCalc.getByHexCode(Utils.hslToHexCode(hsl));
  }
  ...
  static assignColorConstants() {
    ...
    this.BLACK = ColorContrastCalc.HEX_TO_COLOR.get("#000000");
    /** @property {ColorContrastCalc} WHITE - an instance that represents #ffffff */
    this.WHITE = ColorContrastCalc.HEX_TO_COLOR.get("#ffffff");
    /** @property {ColorContrastCalc} GRAY - an instance that represents #808080 */
    this.GRAY = ColorContrastCalc.NAME_TO_COLOR.get("gray");
  ...
  constructor(rgb, name = null) {
    ...
    this.relativeLuminance = calc.relativeLuminance(this.rgb);
  ...
  contrastRatioAgainst(color) {
    ...
    if (!(color instanceof Color)) {
      return calc.contrastRatio(this.rgb, color);
    }

    return calc.luminanceToContrastRatio(this.relativeLuminance,
...
class ColorContrastCalc {
  ...
  static getByHexCode(code) {
    ...
    return this.HEX_TO_COLOR.get(hexCode) || new Color(hexCode);
  }
  ...
  static colorsWithSufficientContrast(color, level = "AA") {
    const ratio = color.levelToContrastRatio(level);
