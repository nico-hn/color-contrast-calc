...
(function() {
  ...
  class Sorter {
    ...
    static compareRgb(rgb1, rgb2, rgbPos = [0, 1, 2], compFuncs = []) {
      for (let i of rgbPos) {
        const result = (compFuncs[i] || this.ascendComp)(rgb1[i], rgb2[i]);
    ...
    static chooseCompFunc(colorOrder) {
      const primaryColors = colorOrder.split("").sort().reverse();
