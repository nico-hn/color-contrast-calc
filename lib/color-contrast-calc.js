  ...
  class Sorter {
    static compareFunction(colorOrder = "rgb") {
      return function(color1, color2) {
        return Sorter.compareRgb(color1.rgb, color2.rgb, colorOrder);
      };
    }

    static compareRgb(rgb1, rgb2, colorOrder = "rgb") {
      let result = 0;

      for (let i of Sorter.primaryColorPos(colorOrder)) {
        const result = rgb1[i] - rgb2[i];
    ...
    static primaryColorPos(colorOrder) {
      return colorOrder.split("").map((primary) => {
