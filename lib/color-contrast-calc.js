...
class ColorContrastCalc {
  ...
  /**
   * Returns a function to be used as a parameter of ColorContrastCalc.sort()
   * @param {string} colorOrder - A left side primary color has a higher sorting precedence
   */
  static compareFunction(colorOrder = "rgb") {
    return this.Sorter.compareFunction(colorOrder);
  }
  ...
  /**
   * Function to be used as a parameter of Array.prototype.sort()
   * @param {ColorContrastCalc} color1 - A color to be compared
   * @param {ColorContrastCalc} color2 - Another color to be compared
   */
  static compare(color1, color2, colorOrder = "rgb") {
    return (this.compareFunction(colorOrder))(color1, color2);
  }
...
(function() {
  ...
  class Sorter {
    static sort(colors, colorOrder = "rgb", keyMapper = null, mode = "auto") {
      let compare = this.compareFunction(colorOrder);
      const keyType = keyMapper ? keyMapper(colors[0]) : colors[0];
      const isStringKey = ColorContrastCalc.isString(keyType);

      if (mode === "hex" || mode === "auto" && isStringKey) {
        compare = this.compareHexFunction(colorOrder);
      }

      return colors.slice().sort(this.composeFunction(compare, keyMapper));
    }
    ...
    static compareFunction(colorOrder = "rgb") {
      ...
      return function(color1, color2) {
        return Sorter.compareRgb(color1.rgb, color2.rgb, rgbPos, compFuncs);
      };
    }
    ...
    static compareHexFunction(colorOrder = "rgb") {
      ...
      return function(hex1, hex2) {
        return Sorter.compareHex(hex1, hex2, rgbPos, compFuncs, rgbCache);
      };
    }

    static compareRgb(rgb1, rgb2, rgbPos = [0, 1, 2],
    ...
    static compareHex(hex1, hex2, rgbPos, compFuncs, rgbCache) {
      const rgb1 = rgbCache.get(hex1) || ColorContrastCalc.hexCodeToDecimal(hex1);
      const rgb2 = rgbCache.get(hex2) || ColorContrastCalc.hexCodeToDecimal(hex2);

      return this.compareRgb(rgb1, rgb2, rgbPos, compFuncs);
    }
    ...
    static chooseCompFunc(colorOrder) {
      ...
      return primaryColors.map(primary => {
        if (ColorContrastCalc.isUpperCase(primary)) {
